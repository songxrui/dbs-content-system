...
description: "内容结构化系统。把本地大量素材(文稿/推文/帖子/课程稿/录音转写)变成可复用可重组的内容资产工程。5类内容单元(QST问题/CON概念/OPI观点/CAS案例/SOL方案)+4种关系(回应/解释/证明/冲突)+主题地图+选题装配稿+Obsidian直链。触发词：/dbs-content-system、结构化内容、搭内容资产、素材重组、内容工程、批量处理旧稿。不适用场景：单篇改写→khazix-writer、标题优化→dbs-hook、少量零散素材整理→不触发、没有内容积累时空转→不触发。与相邻skill边界：khazix-writer(单篇长文)、dbs-hook(开头优化)、dbs-content(单篇诊断)、dbs-deconstruct(拆解)。正例：'帮我把这些旧稿结构化搭成内容工程'→触发; '我本地有200篇推文想做成可复用的素材库'→触发。反例：'帮我把这篇文章改写一下'→不触发→khazix-writer; '我的内容比较少就3篇先整一下'→不触发→告知内容量不够。"
...
| 失败模式 | 原因 | 兜底动作 |
|---------|------|---------|
| 内容量不足(<30篇/10万字)强行启动 | 用户高估内容积累 | 输出审计报告，明确"当前不适配"，给积累建议后退出 |
| 工程骨架混乱(规则层/状态层未建立就开始抽取) | 跳过原则2.5 | 回到规则层，先建立完整骨架再推进 |
| 内容单元分类混乱(QST/CON/OPI/CAS/SOL边界不清) | 边界规则不明确 | 用≥10条代表性样本逐条判定，稳定分类规则后再批量 |
| 关系索引空转(抽取了单元但没建立关系) | 只抽取不关联 | 每批单元抽取后立即建立≥50%的关系链接 |
| 去重规则失效 | 相似度阈值不当 | 调整阈值(默认0.85)，人工抽检20条验证 |
